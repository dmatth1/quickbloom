// bloom_unified.c -- SBBF Bloom filter + software prefetch in bulk paths.
//
// Recommended as the **default** when you don't know the filter size at
// construction time, or you want one design that performs well across the
// cache hierarchy. This is bloom_single_key.c's algorithm with light
// prefetch lookahead added to the bulk paths.
//
// Differences from bloom_single_key.c:
//   - bulk_insert, bulk_contains, and prehash bulk variants prefetch
//     8 keys ahead. Cost in-cache is negligible; benefit out-of-cache
//     is large.
//   - Single-key bloom_insert / bloom_contains are unchanged (no prefetch
//     opportunity for a one-shot call).
//
// Trade-offs vs the alternatives:
//   - vs bloom_single_key.c: slight in-cache regression (~15% on S)
//     in exchange for clear wins at M, L, XL.
//   - vs bloom_batched.c: simpler API (no batch8 needed) and wins at
//     S/M; loses slightly to bloom_batched at XL.
//
// Requirements: x86_64 with AVX2 + BMI2. Same broad CPU target.
//
// On Sapphire Rapids @ 2.1 GHz with clang -O3, prehash:
//   S (128 KB):  ~1.3 ns/op  (~10-15% slower than single_key)
//   M (2 MB):    ~1.6 ns/op  (wins over single_key)
//   L (32 MB):   ~5-6 ns/op  (matches single_key, close to batched)
//   XL (512 MB): ~15-17 ns/op (wins over single_key, close to batched)
...
// Lookahead of 8 keys = 2 iterations of 4-way unrolled bulk.
// Light enough that in-cache overhead is essentially free; deep enough
// to hide ~80 ns of DRAM latency at ~20 ns/iter.
...
void bloom_insert_bulk(void* p, const uint8_t* keys, size_t klen, size_t n) {
    ...
    if (klen == 16) {
        for (; i + 4 <= n; i += 4) {
            ...
            // Prefetch blocks for iter+LOOKAHEAD if in range.
            ...
            // Sequential L+O+S (v14's correctness pattern).
            #define APPLY(P, M) do {                                          \
                __m256i c = _mm256_load_si256((__m256i*)(P));                 \
                _mm256_store_si256((__m256i*)(P), _mm256_or_si256(c, M));     \
            } while (0)
...
// Prehashed ABI -- skip hash recomputation for prefetch (hashes given).
